...
async def download_data(
    ...
    """Downloads the `file_limit` files and saves them as JSON.GZ inside the folder data/20231101.
    The files are saved in their original format for later processing."""
    ...
    # delete old files
    for f in glob.glob(os.path.join(download_dir, "*.json.gz")):
        os.remove(f)
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        # Salvar o arquivo diretamente como json.gz
                        with open(file_path, 'wb') as f:
                            f.write(response.content)
                        logger.info(f"Successfully saved {file_name}")
                    except Exception as e:
                        logger.error(f"Error saving {file_url}: {str(e)}")
                else:
    ...
    logger.info(f"Successfully downloaded {len(glob.glob(os.path.join(download_dir, '*.json.gz')))} files")
...
async def prepare_data() -> str:
    ...
    json_files = glob.glob(os.path.join(raw_dir, "*.json.gz"))

    # Usar os arquivos JSON para o processamento
    files = json_files
